...
import { generateComplaintsPDF } from "../utils/pdf.js";
...
const WardenComplaints = () => {
      ...
      if (response.ok) {
        setComplaints((prevComplaints) =>
          prevComplaints.map((complaint) =>
            complaint.id === id
              ? { ...complaint, is_completed: !complaint.is_completed }
              : complaint
          )
        );
        navigate("/dashboard");
  ...
  const handleDownloadPDF = () => {
    generateComplaintsPDF(complaints);
};
  ...
  return (
    <>
      ...
      >
        Download Pending Complaints PDF
      </button>
        {complaints.length === 0 ? (
            ...
            {complaints.map((complaint) => (
              ...
              >
                <div className="text-lg mb-2 font-semibold text-gray-900 hover:text-black sm:mb-1.5 sm:text-2xl">
                  {complaint.name} (Block-{complaint.block_id},Room No.{complaint.room})
                </div>
                <p className="text-sm">
